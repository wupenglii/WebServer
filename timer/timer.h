...
public:
    ...
    void adjust_timer(util_timer* timer){
        ...
        /*如果目标定时器不是链表的头节点，则将该定时器从链表中取出，然后插入其原来所在位置后
        的部分链表中*/
        else{
    ...
    void del_timer(util_timer* timer){
        ...
        /*如果链表中至少有两个定时器，且目标定时器是链表的头节点，则将链表的头结点重置为原头结点
        的下一个结点，然后删除目标定时器*/
        if(timer == head){
            head = head->next;
            head->prev = NULL;
        ...
        /*如果链表中至少有两个定时器，且目标定时器是链表的尾节点，则将链表的尾结点重置为原头结点
        的下一个结点，然后删除目标定时器*/
        if(timer == tail){
            tail = tail->prev;
            tail->next = NULL;
        ...
        /*如果目标定时器位于链表的中间，则把它前后的定时器串连起来，然后删除目标定时器*/
        timer->prev->next = timer->next;
        timer->next->prev = timer->prev;
        delete timer;
    }
    ...
    void tick(){
        ...
        while(tmp){
            /*因为i每个定时器都使用绝对时间作为超时值，所以我们可以把定时器的超时值和系统当前时间，
            比较以判断定时器是否到期*/
            if(cur < tmp->expire){
                break;
            }
